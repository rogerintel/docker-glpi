...
[[ ! "$VERSION_GLPI" ]] \
	&& VERSION_GLPI=$(curl -s --insecure https://api.github.com/repos/glpi-project/glpi/releases/latest | grep tag_name | cut -d '"' -f 4)
...
SRC_GLPI=$(curl -s --insecure https://api.github.com/repos/glpi-project/glpi/releases/tags/${VERSION_GLPI} | jq .assets[0].browser_download_url | tr -d \")
...
then
	...
	wget --no-check-certificate -P ${FOLDER_WEB} ${SRC_GLPI}
